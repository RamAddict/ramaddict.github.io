...
const startInterval = (): void => {
  if (props.auto) {
    if (props.initialDelay && props.initialDelay > 0) {
...
watch(
  () => [props.auto, props.rotationInterval, props.initialDelay] as const,
  () => {
    cleanupInterval();
    startInterval();
  }
...
<template>
  <Motion
    tag="span"
    :class="cn('relative flex flex-wrap whitespace-pre-wrap', mainClassName)"
    v-bind="$attrs"
    :transition="transition"
    layout
  >
